...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        sampled_ray_bundle = model.sampler.forward(ray_bundle)
        sample_points_reshaped = sampled_ray_bundle.sample_points.reshape(-1, 3).unsqueeze(0)

        # TODO (Q1.4): Visualize sample points as point cloud
        if cam_idx == 0 and file_prefix == '':
            render_points('images/sample_points.png', sample_points_reshaped)
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        if (
            ...
            with torch.no_grad():
                ...
                imageio.mimsave(f'images/{cfg.save_filename}', [np.uint8(im * 255) for im in test_images], loop=0)
