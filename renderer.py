...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        dsigma = -deltas*rays_density
        L_weights = 1 - dsigma.exp()
        Ts = torch.zeros_like(dsigma)
        Ts[..., 1:, :] = dsigma.cumsum(dim=-2)[..., :-1, :]
        Ts = Ts.exp()
        weights = L_weights * Ts
        # sums = weights.sum(dim=-2, keepdim=True) + 0.01
        # weights /= sums
    ...
    def _aggregate(
        ...
        feature = (rays_feature * weights).sum(dim=-2)
...
class SphereTracingRenderer(torch.nn.Module):
    ...
    def sphere_tracing(
        self,
        implicit_fn,
        origins: torch.Tensor, # Nx3
        directions: torch.Tensor, # Nx3
    ):
        '''
        Input:
        ...
        eps = 1e-5
        dirs = directions / directions.norm(dim=-1, keepdim=True)
        points = origins + dirs * self.near
        dists = implicit_fn(points)
        for i in range(self.max_iters):
            points += dirs * dists 
            dists = implicit_fn(points)
        mask = (dists < eps).view(-1, 1)
...
def sdf_to_density(signed_distance, alpha, beta):
    ...
    s = -signed_distance
    psi = torch.zeros_like(s)
    cond = s <= 0
    psi[cond] = 0.5 * torch.exp(s[cond] / beta)
    psi[~cond] = 1 - 0.5 * torch.exp(-s[~cond] / beta)
    return alpha * psi
