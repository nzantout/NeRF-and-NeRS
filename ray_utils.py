...
def get_pixels_from_image(image_size, camera: CamerasBase):
    W, H = image_size[0], image_size[1]

    # TODO (Q1.3): Generate pixel coordinates from [0, W] in x and [0, H] in y
    x = torch.linspace(0.5, W-0.5, W, device=camera.device)
    y = torch.linspace(0.5, H-0.5, H, device=camera.device)
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    xy_grid_sub = xy_grid[torch.randperm(xy_grid.shape[-2], device=xy_grid.device)[:n_pixels]]
...
def get_rays_from_pixels(xy_grid: torch.Tensor, image_size, camera: CamerasBase):
    W, H = image_size[0], image_size[1]

    # TODO (Q1.3): Map pixels to points on the image plane at Z=1
    ndc_points = xy_grid.to(camera.device)
    ...
    rays_o = camera.get_camera_center().repeat(ndc_points.shape[0], 1)
